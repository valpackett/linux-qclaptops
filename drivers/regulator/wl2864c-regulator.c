...
/*
 * Will Semiconductor WL2864C Multi-Output Regulator Driver
...
static int wl2864c_regulator_init(struct wl2864c *chip)
{
	...
	for (id = 0; id < WL2864C_MAX_REGULATORS; id++) {
		...
		dev_err(chip->dev,
			"wl2864c_regulator_init: LDO%d, default:%d", id, initial_voltage[id]);
...
static int wl2864c_i2c_probe(struct i2c_client *client)
{
	...
	dev_info(dev, "wl2864c_i2c_probe Enter...\n");

	cs_gpiod = devm_gpiod_get_optional(dev, "semi,cs",
					   GPIOD_OUT_HIGH |
						GPIOD_FLAGS_BIT_NONEXCLUSIVE);
	if (IS_ERR(cs_gpiod))
	...
	if (!chip) {
		dev_err(dev, "wl2864c_i2c_probe Memory error...\n");
		return -ENOMEM;
	}
...

	{
		...
		dev_info(dev, "wl2864c chip rev: %02x\n", chip_rev);
	}
	...
	dev_info(dev, "wl2864c_i2c_probe Exit...\n");
...
static const struct of_device_id wl2864c_of_match[] = {
	{ .compatible = "willsemi,wl2864c", },
	{ }
};
MODULE_DEVICE_TABLE(of, wl2864c_of_match);

static struct i2c_driver wl2864c_regulator_driver = {
	.driver = {
		.name = "wl2864c-regulator",
		.probe_type = PROBE_PREFER_ASYNCHRONOUS,
		.of_match_table = wl2864c_of_match,
	},
